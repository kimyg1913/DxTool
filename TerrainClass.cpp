...
TerrainClass::TerrainClass() : m_isCreated(false),m_pTerrainShader(nullptr), m_vRecentPicked(987654321, 987654321, 987654321), m_iRecentIndex(-987654321),
m_pTerrainVertex(nullptr), m_pTerrainIndex(nullptr), m_pVB(nullptr), m_pIB(nullptr)
, m_pTexDiffuse(nullptr), m_pTexHeight(nullptr), m_heightMap(nullptr), m_iHighestY(-1), m_iLowestY(1), m_fLODRatio(0.f)
{
...
{
	BitmapClass::DestroyInstance();
	QuadTreeClass::DestroyInstance();
	ErosionClass::DestroyInstance();
	m_vPolygonIndexPerVertex.clear();
	ShutDown();
}

bool TerrainClass::Initialize(LPDIRECT3DDEVICE9 device, int xNumber, int zNumber, int xSize, int zSize, bool isLoadMap, LPCWSTR str, float fLODRatio)
{
	m_isCreated = true;
	m_iCx = xNumber;
	m_iCz = zNumber;
	...
	if (FAILED(D3DXCreateTextureFromFile(device, L"Textures/desert.jpg", &m_pTexDiffuse)))
	{
		return false;
	}
	...
	m_pLight->SetPosition(0, 500.f, -100);
	...
	/*m_pDepthShader = new DepthShaderClass;

	if (!m_pDepthShader->Initialize(device))
	{
		return false;
	}
	...
	ErosionClass::GetInstance()->Init(m_iCx, m_iCz);
	QuadTreeClass::GetInstance()->Init(m_iCx, m_iCz);

	//BitmapClass::GetInstance()->MakeDataFile("HeightMap123.dat", m_iCx, m_iCz);
	//BitmapClass::GetInstance()->LoadDataFile("HeightMap123.dat");

	QuadTreeClass * m_pQuad = QuadTreeClass::GetInstance();
	int a = 0;
	return true;
}
...
bool TerrainClass::LoadHeightMap(LPDIRECT3DDEVICE9 device, LPCWSTR fileName)
{
	...
	if (FAILED(D3DXCreateTextureFromFileEx(device, fileName,
...
bool TerrainClass::MakeDataFile(TCHAR * fileName)
{
		fstream file;
...
LPDIRECT3DTEXTURE9 TerrainClass::GetDepthTex()
{
	return m_pDepthShader->GetShaderTex();
}
...
bool TerrainClass::InitVertex(LPDIRECT3DDEVICE9 device)
{
	...
	if (!m_heightMap)
	{
		...
		perlin->Setup(35, 0.5, 0.015, 5);

		double maxVal = -987654321;
		...
		for (int z = 0; z < m_iCz; z++)
		{
			for (int x = 0; x < m_iCx; x++)
			{
				...
				float noiseValue = perlin->ComputeNoise2D(x, z);
				
				//m_heightMap[index].y = pow(a, 2);

				maxVal = noiseValue > maxVal ? noiseValue : maxVal;
				minVal = noiseValue < minVal ? noiseValue : minVal;

				m_heightMap[index++].y = noiseValue;
			}
		}
		...
		if (minVal < 0)
		{
			for (int i = 0; i < m_iCx * m_iCz; ++i)
			{
					m_heightMap[i].y -= minVal;
			}
		}

		PerlinClass::DestroyInstance();
	}
...
bool TerrainClass::Picking(LPDIRECT3DDEVICE9 device, HWND hwnd, D3DXMATRIX * matWorld, D3DXMATRIX * matView, D3DXMATRIX * matProj, D3DXVECTOR3 * vPicked, BRUSHOREROSION eBEMODE)
{
	...
	else if(eBEMODE == BE_EROSION)
		radius = ErosionClass::GetInstance()->GetRainRadius();
	...
	for (int i = 0; i < m_indexCount; ++i)
	{
		...
		if (isPicked)
		{
			...
			m_vRecentPicked = v0 + u * (v1 - v0) + v*(v2 - v0);
			m_iRecentIndex = i;
			*vPicked = m_vRecentPicked;
			...
			int lRadius = leftTopIndex % m_iCx < radius ? leftTopIndex % m_iCx : radius;
			int rRadius = m_iCx - rightTopIndex % m_iCx <= radius ? m_iCx - rightTopIndex % m_iCx - 1 : radius;

			int uRadius = leftTopIndex / m_iCx < radius ? leftTopIndex / m_iCx : radius;
			...
			for (int i = 0; i <= z; ++i)
			{
				...
				for (int j = 0; j <= x; ++j)
				{
					...
					D3DXVECTOR3 v1 = m_vRecentPicked - v0;
					float length = sqrt(v1.x * v1.x + v1.z * v1.z);

					if (length < radius * 0.75f)
					{
...
bool TerrainClass::Render(LPDIRECT3DDEVICE9 device, D3DXMATRIX * world, D3DXMATRIX * view, D3DXMATRIX * proj)
{
	...
	////인덱스 세팅
	int mTriangles = QuadTreeClass::GetInstance()->GenerateIndex(m_fLODRatio);
	device->SetIndices(m_pIB);

	//m_pShadworShader->RenderShader(device, world, m_pLight->GetViewMatrix(), m_pLight->GetProjMatrix(), m_vertexCount, mTriangles);
	//m_pDepthShader->RenderShader(device, world, view, proj, m_vertexCount, mTriangles);
	...
	m_pTerrainShader->RenderShader(device, world, view, proj, m_vertexCount, mTriangles, m_pTexDiffuse,&m_pLight->GetDiffuseColor(),
		&m_pLight->GetPosition(), m_pLight->GetViewMatrix(), m_pLight->GetProjMatrix() , nullptr);
...
void TerrainClass::ModifyAllNormalVertex()
{
...
void TerrainClass::ModifyPickedVertexNormal()
{
	...
	changedPolygonIndex.clear();

	m_pVB->Unlock();

}

bool TerrainClass::LoadDataMap(LPCWSTR str)
{
	fstream fs;
